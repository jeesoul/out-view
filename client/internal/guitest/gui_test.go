// Package guitest provides headless-compatible tests for GUI logic.
// These tests verify config save/load, state display logic, and tray state
// management without requiring a display server (OpenGL/X11).
package guitest
...
// guiConfig mirrors the config structure saved/loaded by the GUI.
...
func TestGUI_ConfigSaveLoad(t *testing.T) {
	...
	if len(loaded.WebRTC.STUNServers) != len(original.WebRTC.STUNServers) {
		t.Errorf("STUNServers count: got %d, want %d", len(loaded.WebRTC.STUNServers), len(original.WebRTC.STUNServers))
	}
	if loaded.WebRTC.WebRTCTimeout != original.WebRTC.WebRTCTimeout {
		t.Errorf("WebRTCTimeout: got %q, want %q", loaded.WebRTC.WebRTCTimeout, original.WebRTC.WebRTCTimeout)
	}
}
